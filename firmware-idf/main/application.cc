...
#error "Only one of CONFIG_USE_DEVICE_AEC, CONFIG_USE_SERVER_AEC, or CONFIG_USE_SOFTWARE_AEC can be enabled"
    ...
    // Create listening timeout timer (10 seconds of silence -> auto return to IDLE)
    esp_timer_create_args_t listening_timeout_args = {
        .callback = [](void* arg) {
            Application* app = (Application*)arg;
            xEventGroupSetBits(app->event_group_, MAIN_EVENT_LISTENING_TIMEOUT);
        },
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
        .name = "listening_timeout",
        .skip_unhandled_events = true
    };
    esp_timer_create(&listening_timeout_args, &listening_timeout_timer_);
}
...
Application::~Application() {
    ...
    if (listening_timeout_timer_ != nullptr) {
        esp_timer_stop(listening_timeout_timer_);
        esp_timer_delete(listening_timeout_timer_);
    }
...
void Application::Run() {
    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_LISTENING_TIMEOUT) {
            HandleListeningTimeoutEvent();
        }
...
void Application::InitializeProtocol() {
    ...
    if (ota_->HasMqttConfig()) {
        ...
        // Use ElatoProtocol for ElatoAI backend
        protocol_ = std::make_unique<ElatoProtocol>();
    } else {
        // Default to ElatoProtocol for ElatoAI
        ESP_LOGI(TAG, "Using ElatoProtocol for ElatoAI backend");
        protocol_ = std::make_unique<ElatoProtocol>();
    }
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateListening:
            ...
            // Start listening timeout timer (10 seconds of silence -> auto return to IDLE)
...
// Listening timeout timer constants
#define LISTENING_TIMEOUT_MS 10000  // 10 seconds

void Application::StartListeningTimeoutTimer() {
    ESP_LOGI(TAG, "Starting listening timeout timer (%d ms)", LISTENING_TIMEOUT_MS);
    esp_timer_start_once(listening_timeout_timer_, LISTENING_TIMEOUT_MS * 1000);
}

void Application::StopListeningTimeoutTimer() {
    esp_timer_stop(listening_timeout_timer_);
}

void Application::ResetListeningTimeoutTimer() {
    if (GetDeviceState() == kDeviceStateListening) {
        ESP_LOGI(TAG, "Resetting listening timeout timer");
        esp_timer_stop(listening_timeout_timer_);
        esp_timer_start_once(listening_timeout_timer_, LISTENING_TIMEOUT_MS * 1000);
    }
}

void Application::HandleListeningTimeoutEvent() {
    ESP_LOGI(TAG, "Listening timeout - no voice detected for %d seconds", LISTENING_TIMEOUT_MS / 1000);

    auto state = GetDeviceState();
    if (state == kDeviceStateListening) {
        // Close the audio channel and return to idle
        if (protocol_) {
            protocol_->SendStopListening();
            protocol_->CloseAudioChannel();
        }
        ...
        auto display = Board::GetInstance().GetDisplay();
        display->ShowNotification(Lang::Strings::SERVER_TIMEOUT);
    }
}
